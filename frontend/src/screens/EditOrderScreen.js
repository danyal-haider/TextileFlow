...
const EditOrderScreen = ({ navigation, route }) => {
    ...
    // Form Fields (Pre-populated from order)
    ...
    const [deadlineSelected, setDeadlineSelected] = useState(!!order.deadline);
    const [showDatePicker, setShowDatePicker] = useState(false);

    // Enhanced Specs Fields
    const [category, setCategory] = useState(order.category || ['T-Shirts / Tops']);
    const [gender, setGender] = useState(order.gender || 'Unisex');
    const [selectedSizes, setSelectedSizes] = useState(order.sizes || []);
    const [sizeBreakdown, setSizeBreakdown] = useState(order.sizeBreakdown || '');
    const [customizationType, setCustomizationType] = useState(order.customizationType || 'None');
    const [logoPlacement, setLogoPlacement] = useState(order.logoPlacement || '');
    const [noOfLogos, setNoOfLogos] = useState(order.noOfLogos?.toString() || '');
    const [targetPrice, setTargetPrice] = useState(order.targetPrice?.toString() || '');
    const [sampleRequired, setSampleRequired] = useState(!!order.sampleRequired);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert('Error', error.response?.data?.message || 'Failed to update order. Please try again.');
        } finally {
            setLoading(false);
        }
    };
